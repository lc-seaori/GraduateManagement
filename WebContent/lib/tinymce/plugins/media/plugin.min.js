...
tinymce.PluginManager.add('media', function(editor, url) {
	...
	function showDialog() {
		...
		if (editor.settings.media_alt_source !== false) {
			generalFormItems.push({name: 'source2', type: 'filepicker', filetype: 'media', size: 40, label: '备用地址'});
		}

		if (editor.settings.media_poster !== false) {
			generalFormItems.push({name: 'poster', type: 'filepicker', filetype: 'image', size: 40, label: '封面'});
		}
		...
		if (editor.settings.media_dimensions !== false) {
			generalFormItems.push({
				...
				items: [
					...
					{name: 'constrain', type: 'checkbox', checked: true, text: '约束比例'}
				]
			});
		}
