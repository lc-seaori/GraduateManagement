...
<div class="easyui-panel" style="padding: 30px 70px 50px 70px" align="center">
	<s:form id="editOrganizationForm" namespace="/" method="post" theme="simple" enctype="multipart/form-data">
		<s:hidden name="id"/>
	...
	<div style="text-align: center; padding: 5px;">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" 
			onclick="toEditOrganizations('#editOrganizationForm')">更新</a>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-no" 
			onclick="exitEditOrganizationPage()">关闭</a>
	</div>
</div>
<script type="text/javascript">
  	...
  	$(function(){
		...
		//初始化下拉树(combotree)
  		$('#editOrganizationForm').find('#pid').combotree({
  			lines : true,
			url : 'Organization_doNotNeedSession_tree.action',
			onLoadSuccess:function(row,data){
				//数据异步加载完成执行这个
				var parentId=$('#editOrganizationForm').find('#parentId').val();
				$('#editOrganizationForm').find('#pid').combotree('setValue',parentId);
			}
  		});
  	});

	function exitEditOrganizationPage() {
		$.messager.confirm('请确认','确认放弃修改，离开修改页面？',function(r){
			if(r){
				$('#editOrganizationDialog').dialog('close');
			}
		});
	}
	
	//表单提交
	function toEditOrganizations(form){
    	//在提交数据之前执行表单验证
		...
		$(form).ajaxSubmit({
		     type: 'post',
		     url: "Organization_doEditOrganization.action",
		     success: 
		    	function(data){
		    		...
		    		if (result.success){
						...
						$.messager.alert("操作提示","修改组织部门信息失败" ,"error");
					}
				}
		});    
	}
