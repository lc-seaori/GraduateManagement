...
<div class="easyui-panel" style="padding: 30px 70px 50px 70px" align="center">
	<s:form id="addAuthForm" namespace="/" method="post" theme="simple" enctype="multipart/form-data">
		<s:hidden name="id"/>
		<table cellpadding="7">
			<tr>
				<td>权限名称：</td>
...
<script type="text/javascript">
  	...
  	$(function(){
		...
		if(!String.prototype.trim) {
  		...
  		//初始化下拉树(combotree)
  		$('#addAuthForm').find('#pid').combotree({
  			lines : true,
			url : 'System_Auth_doNotNeedSession_tree.action'
  		});
  	});

	function exitAddAuthPage() {
	...
	function toAddAuths(form){
		...
		$(form).ajaxSubmit({
		     type: 'post',
		     url: "System_Auth_doAddAuth.action",
		     success: 
		    	function(data){
		    		var result=eval("("+data+")");
		    		if (result.success){
					   	$.messager.alert("操作提示", "添加权限成功","info");  
					   	//重新加载
					   	$('#authList').treegrid('reload');
					   	//关闭添加页面
					   	closeTab(getCurrentTab());
					} else{
						$.messager.alert("操作提示","添加权限失败" ,"error");
					}
				}
		});    
	}
