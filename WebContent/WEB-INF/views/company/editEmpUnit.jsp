...
<div class="easyui-panel" style="padding: 30px 70px 50px 70px" align="center">
	<s:form id="editEmpUnitForm" namespace="/" method="post" theme="simple" enctype="multipart/form-data">
	...
	<div style="text-align: center; padding: 5px;">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" 
			onclick="editEmpUnit('#editEmpUnitForm')">更新</a>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-no" 
			onclick="exitEditEmpUnitPage()">关闭</a>
	</div>
</div>
<script type="text/javascript">
  	...
  	$(function(){
		...
		if(!String.prototype.trim) {
  		...
  		$('#editEmpUnitForm').find('#unitCode').change(function(){
  			var unitCode = $(this).val().trim();
			var url="EmployingUnit_Edit_validateUnitCode";
	...
	//表单提交
	function editEmpUnit(form){
    	//在提交数据之前执行表单验证
		$('#editEmpUnitForm').form('enableValidation').form('validate'); 
		...
		$(form).ajaxSubmit({
		     type: 'post',
		     url: "EmployingUnit_Edit_editEmpUnit.action" ,
		     success: 
		    	function(data){
