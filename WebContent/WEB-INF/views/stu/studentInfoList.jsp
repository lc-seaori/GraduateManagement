...
<div style="width: 99%; height: auto">
	<div id="studentInfoToolbar" style="padding: 5px; height: auto">	
			<td>准考证号:</td>
			...
			<td><input class="easyui-datebox" id="graduateTime" 
				...
				<a style="height: 25px" class="easyui-linkbutton" href="javascript:void(0)" onclick="$('#studentInfoToolbar').find('#graduateTime').combobox('clear');" >清空</a>
			</td>
			
			<button href="#" class="easyui-linkbutton" plain="true" 
				onclick="getStudentInfosByLimit('#studentInfoToolbar', '#studentInfoList')" 
				iconCls="icon-search">查询</button>
	</div>
	<table id="studentInfoList" class="easyui-datagrid" style="width: auto;"
		pagination="true" url="StudentInfo_List_getStudentList.action" toolbar="#studentInfoOperToolbar" 
    ...
    <div id="studentInfoOperToolbar">
    	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="toAddNewStudentInfo()">添加</a>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="toEditStudentInfo()">编辑</a>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-clear" plain="true" onclick="toDeleteStudentInfo()">删除</a>
   		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-filter" plain="true" onclick="toApplyDepartmentAduitStu()">申请院系审核</a>
   		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-filter" plain="true" onclick="toCancelApplyDepartmentAduitStu()">取消申请院系审核</a>
    </div>
</div>
<script type="text/javascript">
	...
	$(function(){
		//温馨提示
		$('#studentInfoToolbar').find('#graduateTime').datebox({
	...
	/* $(function(){
		var examNum=$("#studentInfoToolbar").find("#examNum").val().trim();
		var name=$("#studentInfoToolbar").find("#name").val().trim();
		var idCard=$("#studentInfoToolbar").find("#idCard").val().trim();
		var graduateTime=$("#studentInfoToolbar").find('#graduateTime').combobox('getValue');
		$('#studentInfoList').datagrid({
			queryParams: {
				examNum: examNum,
				name: name,
				idCard: idCard,
	...
	function getStudentInfosByLimit(toolbar,datagrid){
		...
		//再次发送请求
		$.post('StudentInfo_List_getStudentList.action',{
			examNum: examNum,
			name:  name,
			idCard: idCard,
			graduateTime:graduateTime
	...
	//添加
	function toAddNewStudentInfo(){
		openTab("StudentInfo_AddNewInfo_toAddStudentInfoPage.action",'添加新生信息');
	}
	
	//编辑
	function toEditStudentInfo(){
		var rows = $('#studentInfoList').datagrid('getSelections');
   		if (rows.length == 1) {
   			openTab("StudentInfo_EditStuInfo_toEditStudentInfoPage.action?id="+rows[0].id,'编辑新生信息');
		...
		if(rows.length==0){
			...
			$.messager.confirm('请确认','您确定删除共'+length+'条信息吗?',function(r){
                   if (r){
                   	...
                   	function(data){
                    	if (data.success){
                      		$('#studentInfoList').datagrid('uncheckAll');
                            $.messager.show({    // show error message
                            	title: 'Tips',
                                msg: '删除记录成功'
                            });
                            $('#studentInfoList').datagrid('reload');    // reload the user data
                        }else {
                        	$.messager.show({    // show error message
                            	title: 'Error',
                           		msg: '删除记录失败'
	...
	//申请院系审核
	function toApplyDepartmentAduitStu(){
		var applyRows=$('#studentInfoList').datagrid('getSelections');
		if(applyRows.length==0){
			$.messager.alert('提示', '请勾选申请院系审核的学生信息！', 'info');
   			return;
		}else{
            	...
            	if (r){
                	var params="";
                   	for(i=0;i<applyRows.length;i++){
                   		...
                   		if(applyRows[i].departmentAduit=="审核中"){
                   			$.messager.alert('提示', '勾选的信息中有在【审核中】的信息，请重选选择', 'warning');
                   			return ;
                   		}else if(applyRows[i].departmentAduit=="通过"){
                   	...
                   	//发送请求到action
                   	var url="StudentInfo_AduitStuInfo_toDoApplyDepartmentAduit.action";
   					//alert(url);
                   	$.post(url, {idList:params},
                   	function(data){
                    	if (data.success){
	...
	//取消申请院系审核
	function toCancelApplyDepartmentAduitStu(){
		var applyRows=$('#studentInfoList').datagrid('getSelections');
		if(applyRows.length==0){
			$.messager.alert('提示', '请勾选需要取消申请院系审核的学生信息！', 'info');
   			return;
		}else{
			//alert(applyRows[0].departmentAduit);
			var length=applyRows.length;
            	...
            	if (r){
                	var params="";
                   	for(i=0;i<applyRows.length;i++){
                   		...
                   		if(applyRows[i].departmentAduit=="未审核"){
                   	...
                   	//发送请求到action
                   	var url="StudentInfo_AduitStuInfo_toDoCacelApplyDepartmentAduit.action";
   					//alert(url);
                   	$.post(url, {idList:params},
                   	function(data){
                    	if (data.success){
